...
origin=0                    #原始图 原始图上画原始mask  原始mask标签
augment=0                   #原始图 hsv图              hsv图上画增广mask             增广mask标签
augment_and_origin=1        #原始图 hsv图              hsv图上画增广mask+原始mask    增广mask+原始mask标签
...
def augment_polygon(points, image_width, image_height, iou_threshold=0.1,max_shift=1, drop_prob=0.1, add_prob=):
    """
...
if augment_and_origin:
    ...
    ##############################augment#####################################
        ...
        aug_image_path = os.path.join(output_folder, f"{image_name}_ori_and_augmented.jpg")
        aug_txt_path = os.path.join(output_folder, f"{image_name}_ori_and_augmented.txt")
        hsv_image_path = os.path.join(output_folder, f"{image_name}_hsv.jpg")
        aug_image=image.copy()
